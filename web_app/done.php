  ...
  require_once 'includes/config.php';
   if(loggedin()){ 
	...
	<html>
		...
		$query="SELECT `id`,`notes`,`solution` from `requests` WHERE `is_done`='1' AND `expert_id`='$expert_id'";
    	...
    	<textarea name="comments" class="form-control" rows = '5' id = 'ta'><?php echo $f2; ?></textarea>
		...
		unlink('images/'.$img);
		$query_del1 = "DELETE FROM  requests WHERE id ='$fs_submitted'";
		...
		$query_update = "UPDATE requests SET is_done='0' WHERE id ='$f0_submitted'";
